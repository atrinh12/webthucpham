   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         if($update_image){
            ...
            $message[] = 'Ảnh Được Cập Nhập Thành Công!';
         }
      }
   }
...
<body>
   ...
   ?>
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <input type="text" name="name" placeholder="Nhập Tên Sản Phẩm" required class="box"
                value="<?= $fetch_products['name']; ?>">
            <input type="number" name="price" min="0" placeholder="Nhập Giá Sản Phẩm" required class="box"
                value="<?= $fetch_products['price']; ?>">
            <select name="category" class="box" required>
                <option selected><?= $fetch_products['category']; ?></option>
                <option value="vegitables">Rau Củ Quả</option>
                <option value="fruits">Trái Cây</option>
                <option value="meat">Thịt</option>
                <option value="fish">Cá</option>
            </select>
            <textarea name="details" required placeholder="Nhập Chi Tiết Sản Phẩm" class="box" cols="30"
